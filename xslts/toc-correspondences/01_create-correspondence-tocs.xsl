...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="outdir" as="xs:string" select="'./tocs/'"/>
    <xsl:template match="main">
        ...
        <xsl:for-each
            ...
            <xsl:result-document indent="true" href="{$outdir}{$dateiname}">
    ...
    <xsl:function name="foo:nameOhneKomma" as="xs:string">
        ...
        <xsl:choose>
            <xsl:when test="contains($name, ', ')">
                <xsl:value-of select="concat(substring-after($name, ', ')[1], ' ', substring-before($name, ', ')[1])"/>
            </xsl:when>
            <xsl:otherwise>
